...
class LicenseKeyManager
{
    ...
    public function __construct()
    {
        ...
        $this->stored_license_key_validity_info = $this->loadStoredLicenseKeyValidityInfo();
    }
    ...
    public function isLicenseValidityStatusEligibleForProMode(string $license_validity_status): bool
    {
        if ($license_validity_status === 'valid') {
            return true;
        }

        // @see https://github.com/soflyy/breakdance/issues/5169#issuecomment-1445008521
        if ($license_validity_status === 'expired') {
            return true;
        }
    ...
    public function getSubscriptionModeEligibleForStoredLicenseKey(): string
    {
        ...
        if (
            ...
            && $this->isLicenseValidityStatusEligibleForProMode($this->stored_license_key_validity_info['edd_key_info']['license'])
        ) {
            return 'pro';
        }
    ...
    public function getPluginUpdaterSettingsBasedOnLicenseKeyValidityInfo(): ?array
    {
        ...
        if (!$this->isLicenseValidityStatusEligibleForProMode($this->stored_license_key_validity_info['edd_key_info']['license'])) {
            return null;
        }
    ...
    /**
     * @psalm-return array{product: string, is_valid: string, activation_status: string, expires: string, expires_in_human_readable: string}
     */
     ...
     */
    protected function loadStoredLicenseKeyValidityInfo(): ?array
    {
    ...
    /**
     * @psalm-return null|EddLicenseKeyValidityInfo
     */
    protected function fetchLicenseKeyValidityInfoFromEdd(string $license_key): ?array
    {
        /** @psalm-var null|EddLicenseKeyValidityInfo $license_key_validity_info */
